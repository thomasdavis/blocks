...
Provide a score between 0 and 1, and explain your reasoning.`,
   ...
   * - Visual hierarchy
   * - Layout integrity
   * - All visual elements are clearly visible
   *
   * @example
   * const issues = await ai.validateVisualSemantics({
   *   screenshot: screenshotBuffer,
   *   blockName: 'theme.modern_professional',
   *   visualRules: ['WCAG AA contrast', 'Clear visual hierarchy'],
   *   philosophy: ['Resume themes must prioritize readability']
   * });
   */
  async validateVisualSemantics(params: {
    ...
    const base64Screenshot = params.screenshot.toString('base64');

    const visualRulesText = params.visualRules?.length
      ? `\n\nVISUAL RULES:\n${params.visualRules.map((r) => `- ${r}`).join("\n")}`
      : "";
    ...
    const viewportText = params.viewport
      ...
      messages: [
        {
          ...
          content: [
            {
              type: 'text',
              text: `You are validating visual aspects of the "${params.blockName}" block in the Blocks framework.
${philosophyText}${visualRulesText}${viewportText}
...
1. **Color Contrast** (WCAG AA minimum):
   ...
   - Check text on all backgrounds (headers, body, links, buttons)

2. **Text Readability**:
   - Font sizes appropriate (body text 16px+, headings larger)
   - Line heights comfortable (1.5-1.8 for body text)
   - Adequate spacing between sections

3. **Visual Hierarchy**:
   - Clear distinction between h1, h2, h3 headings
   - Important information visually prominent
   - Logical flow guides the eye
...
- ERROR: Fails WCAG contrast ratios, unreadable text, broken layout
- WARNING: Suboptimal spacing, unclear hierarchy, minor visibility issues`
            },
            {
              type: 'image',
